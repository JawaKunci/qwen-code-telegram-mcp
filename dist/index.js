...
async function fetchBotInfo(botInstance) {
    try {
...
function saveApprovalModes(modes) {
    try {
...
class ConfirmationManager {
    ...
    addPending(chatId, prompt, qwenProcess) {
 ...
 */
function removeBotMentions(text, botUsername) {
    if (!botUsername)
        return text;
 ...
 */
function wasBotMentionedInQuote(quotedContent, botUsername) {
    if (!quotedContent || !botUsername)
        return false;
    const mentionRegex = new RegExp(`@${botUsername}`, 'i');
 ...
 */
function shouldBotRespondInGroup(msg, botUsername) {
...
class SessionManager {
    ...
    async saveSession(chatId, messages) {
        const sessionPath = this.getSessionPath(chatId);
        const session = {
            chatId,
            messages,
...
class AllowlistManager {
    ...
    addAllowed(chatId, username, firstName) {
        const data = this.read();
    ...
    verifyPairingCode(code) {
        const data = this.read();
        const pending = data.pending.find(p => p.code === code.toUpperCase() &&
...
class QwenClient {
    ...
    MAX_CONTEXT_CHARS = 8000; // Limit context to ~8k chars
    constructor(workspacePath) {
     ...
     */
    async chat(message, context = [], chatId) {
            ...
            try {
                ...
                if (context.length > 0) {
                    ...
                    for (let i = context.length - 1; i >= 0; i--) {
                        if (messagesToInclude.length >= this.MAX_CONTEXT_MESSAGES)
                            break;
                        const msgSize = context[i].content.length;
                        if (contextSize + msgSize > this.MAX_CONTEXT_CHARS)
                            break;
     ...
     */
    async chatInteractive(message, chatId, context = []) {
 ...
 */
async function streamResponse(chatId, qwenProcess) {
    ...
    const getProgressiveDelay = () => {
        ...
        if (editCount >= 5)
            return BASE_WAIT + (editCount - 5) * 200;
        // First 5 edits: fast
        return BASE_WAIT;
    };
    return new Promise((resolve) => {
        qwenProcess.stdout.on('data', (chunk) => {
            ...
            if (buffer.length >= MIN_LINES || (now - lastEdit >= waitTime && buffer.length > 0)) {
                bot.editMessageText(fullOutput, { chat_id: chatId, message_id: messageId }).then(() => {
                    editCount++;
                    consecutiveEdits = 0;
                }).catch((err) => {
        ...
        qwenProcess.on('close', () => {
            // Final update
            bot.editMessageText(fullOutput, { chat_id: chatId, message_id: messageId }).catch(() => { });
            resolve(fullOutput);
        });
    });
}
...
bot.on('message', async (msg) => {
    ...
    try {
        ...
        if (processedText.startsWith('/start')) {
            ...
            await bot.sendMessage(chatId, `👋 Welcome to Telegram MCP Bot v2.2.0!\n\n` +
        ...
        if (processedText.startsWith('/pair')) {
            ...
            if (result) {
                ...
                await bot.sendMessage(chatId, `✅ **Pairing successful!**\n\n` +
...
// ============================================================================
async function handleCommand(msg, session, sessionManager) {
    const chatId = msg.chat.id.toString();
    const text = msg.text || '';
    ...
    switch (commandName) {
        ...
        case 'status':
            ...
            await bot.sendMessage(chatId, '📊 Server Status v2.1.6\n\n' +
                '━━━ Server Info ━━━\n' +
        ...
        case 'approval':
            ...
            if (!modeArg) {
                // Show current mode
                await bot.sendMessage(chatId, '📋 **Approval Mode**\n\n' +
                ...
                await bot.sendMessage(chatId, `✅ **Approval mode changed**\n\n` +
                    `**Mode:** \`${modeArg}\`\n` +
                    `**Description:** ${modeDescriptions[modeArg]}\n\n` +
                    `This setting is persistent and will be used for all Qwen Code operations.`);
            }
            else {
                await bot.sendMessage(chatId, '❌ Invalid mode. Available modes: `plan`, `auto-accept`, `yolo`\n\n' +
                    'Usage: `/approvalmode <mode>`\n' +
                    'Example: `/approvalmode yolo`');
            }
            break;
        case 'session':
            ...
            await bot.sendMessage(chatId, `📊 **Session Info**\n\n` +
                `**Chat ID:** \`${chatId}\`\n` +
        ...
        case 'whoami':
            const user = msg.from;
            await bot.sendMessage(chatId, `👤 **Your Info**\n\n` +
                `**ID:** \`${user?.id}\`\n` +
...
// ============================================================================
async function handleFileMessage(msg, chatId, username, firstName) {
    const document = msg.document;
    const photos = msg.photo;
...
setInterval(() => {
    ...
    const cleanupOldFiles = (dir, days = 7) => {
        if (!fs.existsSync(dir))
            return;
        const now = Date.now();
 ...
 */
server.registerTool('list_active_chats', {
 ...
 */
server.registerTool('generate_telegram_pairing_code', {
 ...
 */
server.registerTool('verify_telegram_pairing', {
 ...
 */
server.registerTool('send_telegram_file', {
 ...
 */
server.registerTool('list_telegram_downloads', {
